...
\begin{Proof}[\cref{thm:LU分解的存在性}]
    不妨考虑\cref{pro:高斯消元的流程}中的第$k-1$步，由于$\vb{A}^{(k)}=\vb{M}_{k-1}\cdots\vb{M}_1\vb{A}$
    ...
    这使得\cref{pro:高斯消元的流程}第$k$步的$\tau_i^{(k)}=a_{ik}^{(k)}/a_{kk}^{(k)}$可以进行，这就证明了$\vb{A}=\vb{L}\vb{U}$的存在性！
