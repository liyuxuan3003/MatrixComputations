...
QR分解是适用于$m\geq n$的高矩阵$\vb{A}\in\R^{m\times n}$的分解$\vb{A}=\vb{Q}\vb{R}$，其中，$\vb{Q}\in\R^{m\times m}$是正交矩阵，$\vb{R}\in\R^{m\times n}$是高的上三角矩阵。本质上来说，QR分解其实也是一个将$\vb{A}$通过消元上三角化为$\vb{R}$的过程。消元依赖的就是\cref{sec:两种正交变换}介绍的Householder反射和Givens旋转，它们可以分别将一列或一个元素消为$0$，相乘就构成了$\vb{Q}$。本节先讨论QR分解的存在性和唯一性。
