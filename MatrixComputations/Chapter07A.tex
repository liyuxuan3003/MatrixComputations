\section{半正定矩阵的定义}

讨论Cholesky分解时，我们已经引入过半正定矩阵的定义，这里系统的再定义一遍。

\subsection{二次型}

二次型是由矩阵和向量定义的一个数。

\begin{BoxDefinition}[二次型]
    令$\vb{A}\in\Symm^n$，对于$\vb{x}\in\R^n$，定义二次型（Quadratic Form）
    \begin{Equation}[二次型]
        \vb{x}^T\vb{A}\vb{x}
    \end{Equation}
\end{BoxDefinition}

实际上，二次型可以对于任意的$\vb{A}\in\R^n$定义，并不仅限于$\vb{A}\in\Symm^n$的对称矩阵。不过，由于任意矩阵$\vb{A}\in\R^n$都可以分为对称部分$\vb{H}=(\vb{A}+\vb{A}^T)/2$和反称部分$\vb{S}=(\vb{A}-\vb{A}^T)/2$
\begin{Equation}[二次型对称1]
    \vb{A}=\frac{\vb{A}+\vb{A}^T}{2}+\frac{\vb{A}-\vb{A}^T}{2}=\vb{H}+\vb{S}
\end{Equation}

其中$\vb{H}=\vb{H}^T$而$\vb{S}=-\vb{S}^T$，可以证明，反称部分对二次型贡献为零
\begin{Equation}[二次型对称1]
    \vb{x}^T\vb{A}\vb{x}=\vb{x}^T\qty[(\vb{A}+\vb{A}^T)/2]\vb{x}
\end{Equation}

因此，任意矩阵$\vb{A}$的二次型总等于其对称分量$\vb{H}=(\vb{A}+\vb{A}^T)/2$的二次型。

\subsection{半正定矩阵}

正定的概念和二次型的正负是密切相关的。

\begin{BoxDefinition}[正定矩阵]
    设$\vb{A}\in\Symm^n$正定（Positive Definite, PD），若对于任意$\vb{x}\in\R^n$且$\vb{x}\neq\vb{0}$有$\vb{x}^T\vb{A}\vb{x}>0$。
\end{BoxDefinition}

\begin{BoxDefinition}[半正定矩阵]
    设$\vb{A}\in\Symm^n$半正定（Positive Semidefinite, PSD），若对于任意$\vb{x}\in\R^n$有$\vb{x}^T\vb{A}\vb{x}\geq 0$。
\end{BoxDefinition}

简而言之，正定和半正定就是在讨论矩阵的二次型对于任意的$\vb{x}$是$>0$还是$\geq 0$。对于正定矩阵其定义额外要求$\vb{x}\neq\vb{0}$，因为当$\vb{x}=\vb{0}$时恒有$\vb{x}^T\vb{A}\vb{x}=0$，这样就没有矩阵能是正定的了。

\begin{BoxDefinition}[负定矩阵]
    设$\vb{A}\in\Symm^n$负定（Negative Definite, ND），若对于任意$\vb{x}\in\R^n$且$\vb{x}\neq\vb{0}$有$\vb{x}^T\vb{A}\vb{x}<0$。
\end{BoxDefinition}

\begin{BoxDefinition}[半负定矩阵]
    设$\vb{A}\in\Symm^n$半负定（Negative Semidefinite, NSD），若对于任意$\vb{x}\in\R^n$有$\vb{x}^T\vb{A}\vb{x}\leq 0$。
\end{BoxDefinition}

负定和半负定本质上就是正定和半正定取反：$\vb{A}$负定等价于$-\vb{A}$正定，$\vb{A}$半负定等价于$-\vb{A}$半正定。

\begin{BoxDefinition}[不定矩阵]
    设$\vb{A}\in\Symm^n$不定（Indefinite），若$\vb{A}$与$-\vb{A}$均不是半正定矩阵，即存在$\vb{x},\vb{y}\in\R^n$使得
    \begin{Equation}[不定矩阵]
        \vb{x}^T\vb{A}\vb{x}>0,\qquad \vb{y}^T\vb{A}\vb{y}<0
    \end{Equation}
\end{BoxDefinition}

不定矩阵既不保证二次型始终非负，也不保证始终非正，而是同时存在正值和负值。

\subsection{定性符号}

为了书写方便，引入以下符号表示矩阵的定性：

\begin{BoxDefinition}[矩阵定性符号]
    \begin{align}
        \vb{A} \succ 0 &\quad\Longleftrightarrow\quad \vb{A}\in\Symm^n\text{正定（PD）}\\
        \vb{A} \succeq 0 &\quad\Longleftrightarrow\quad \vb{A}\in\Symm^n\text{半正定（PSD）}\\
        \vb{A} \prec 0 &\quad\Longleftrightarrow\quad \vb{A}\in\Symm^n\text{负定（ND）}\\
        \vb{A} \preceq 0 &\quad\Longleftrightarrow\quad \vb{A}\in\Symm^n\text{半负定（NSD）}
    \end{align}
\end{BoxDefinition}

