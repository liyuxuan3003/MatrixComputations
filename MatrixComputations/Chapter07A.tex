...
\begin{itemize}
    \item $\vb{A}\succ 0$：$\vb{A}$正定（PD）
    \item $\vb{A}\succeq 0$：$\vb{A}$半正定（PSD）
    \item $\vb{A}\prec 0$：$\vb{A}$负定（ND）
    \item $\vb{A}\preceq 0$：$\vb{A}$半负定（NSD）
\end{itemize}
...
\begin{BoxProperty}[半正定矩阵的和]
    若$\vb{A},\vb{B}\succeq 0$，则$\vb{A}+\vb{B}\succeq 0$。特别地，若$\vb{A}\succeq 0$且$\vb{B}\succ 0$，则$\vb{A}+\vb{B}\succ 0$。
\end{BoxProperty}

\begin{BoxProperty}[正定矩阵的逆]
    $\vb{A}\succ 0$当且仅当$\vb{A}^{-1}\succ 0$。
\end{BoxProperty}

\begin{BoxProperty}[半正定矩阵的迹和行列式]
    若$\vb{A}\succeq 0$，则$\tr(\vb{A})\geq 0$且$\det(\vb{A})\geq 0$。若$\vb{A}\succ 0$，则$\tr(\vb{A})>0$且$\det(\vb{A})>0$。
\end{BoxProperty}

\begin{BoxProperty}[半正定矩阵集合的凸性]
    若$\vb{A},\vb{B}\succeq 0$，则对任意$\theta\in[0,1]$有$\theta\vb{A}+(1-\theta)\vb{B}\succeq 0$。
\end{BoxProperty}
