...
\begin{BoxFormula}[部分选主元的LU分解]
    部分选主元的LU分解是指，对于$\vb{A}\in\R^{n\times n}$
...
\begin{BoxAlgorithm}[部分选主元的LU分解]
    ...
    该算法实现$\vb{P}\vb{A}=\vb{L}\vb{U}$的分解。
    
    $\vb{L}\in\R^{n\times n}$是单位下三角矩阵，满足$|l_{ij}|\leq 1$，$\vb{U}\in\R^{n\times n}$是上三角矩阵。
    
    $\vb{P}\in\R^{n\times n}$由$\vb{P}=\vb*{\Pi}_{n-1}\cdots\vb*{\Pi}_{1}$给出，$\vb*{\Pi}_k\in\R^{n\times n}$是交换第$k$行与第$\piv(k)$行。
